"""Echantillons comparatifs des voix Piper francaises, mesures a l'appui.

Piper sert ici un corpus francais et rien d'autre : siwis est une locutrice
suisse romande, upmc vient de la Sorbonne, mls de livres audio francais. Aucune
de ces voix n'a jamais entendu d'anglais, donc l'accent que l'utilisateur a
releve sur Pocket ne peut pas s'y reproduire. Ce banc verifie qu'on ne paie pas
cette nativite en intelligibilite ni en latence.
"""
from __future__ import annotations

import asyncio
import re
import unicodedata
import wave
from pathlib import Path

import numpy as np

SORTIE = Path("/workspace/data/voix")
PHRASE = (
    "Bonjour, je suis là. Prends ton temps, je t'écoute. "
    "Le facteur temps réel doit rester inférieur à un."
)
CANDIDATES = [
    ("siwis", "fr_FR-siwis-medium", None),
    ("upmc-jessica", "fr_FR-upmc-medium", 0),
    ("mls-1840", "fr_FR-mls-medium", 0),
]


def normaliser(texte: str) -> list[str]:
    plat = "".join(
        c for c in unicodedata.normalize("NFD", texte.lower())
        if unicodedata.category(c) != "Mn"
    )
    return re.findall(r"[a-z0-9']+", plat)


def taux_erreur_mot(attendu: str, obtenu: str) -> float:
    a, b = normaliser(attendu), normaliser(obtenu)
    if not a:
        return 1.0
    d = list(range(len(b) + 1))
    for i, mot_a in enumerate(a, 1):
        prec, d[0] = d[0], i
        for j, mot_b in enumerate(b, 1):
            prec, d[j] = d[j], min(d[j] + 1, d[j - 1] + 1, prec + (mot_a != mot_b))
    return d[len(b)] / len(a)


def fondamentale(x: np.ndarray, taux: int) -> float:
    x = x.astype(np.float32) - x.astype(np.float32).mean()
    ac = np.correlate(x, x, "full")[len(x) - 1:]
    lo, hi = int(taux / 400), int(taux / 70)
    return float(taux / (lo + int(np.argmax(ac[lo:hi]))))


def ecrire(chemin: Path, pcm: np.ndarray, taux: int) -> None:
    with wave.open(str(chemin), "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(taux)
        w.writeframes(pcm.astype("<i2").tobytes())


def vers_16k(x: np.ndarray, taux: int) -> np.ndarray:
    if taux == 16000:
        return x.astype(np.float32)
    n = int(len(x) * 16000 / taux)
    return np.interp(
        np.linspace(0, len(x) - 1, n), np.arange(len(x)), x.astype(np.float32)
    ).astype(np.float32)


async def main() -> None:
    from src.ears.faster_whisper_asr import FasterWhisperASR
    from src.mouth.piper_tts import PiperTTS

    SORTIE.mkdir(parents=True, exist_ok=True)
    ears = FasterWhisperASR(model_size="large-v3-turbo", language="fr", device="cuda")
    await ears.load_model()

    print(f"{'voix':16} {'f0':>8} {'TTFA':>8} {'RTF':>6} {'WER':>7}  transcription")
    print("-" * 104)
    for nom, fichier, locuteur in CANDIDATES:
        for profil in ("flat", "ambiante"):
            tts = PiperTTS(
                model_path=f"/workspace/models/piper/{fichier}.onnx",
                speaker_id=locuteur,
                profile=profil,
            )
            if not await tts.load_model():
                print(f"{nom:16} indisponible")
                continue
            out = await tts.synthesize(PHRASE)
            pcm = np.asarray(out["audio"]).ravel()
            taux = int(out["sample_rate"])
            etiquette = f"{nom}-{profil}"
            ecrire(SORTIE / f"piper_{etiquette}_fr.wav", pcm, taux)

            flottant = pcm.astype(np.float32) / 32768.0
            texte = ((await ears.transcribe(vers_16k(flottant, taux))).get("text") or "").strip()
            print(
                f"{etiquette:16} {fondamentale(flottant, taux):6.1f}Hz "
                f"{out.get('ttfa_ms', 0):7.0f}ms {out.get('rtf', 0):6.2f} "
                f"{taux_erreur_mot(PHRASE, texte):6.1%}  {texte[:52]}",
                flush=True,
            )


if __name__ == "__main__":
    asyncio.run(main())
