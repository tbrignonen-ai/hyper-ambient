"""Captures Presence HA — 17 sept. Hors runtime live (pas de session vocale)."""
...
def capturer_app() -> None:
    ...
    try:
        ...
        application.tic()
        _grab(application.racine, DEST_REPO / f"{PREFIX}-5-distant.png")
        _sauver(DEST_REPO / f"{PREFIX}-5-distant.png")
    finally:
        application.fermer()
...
def capturer_overlay() -> None:
    ...
    try:
        ...
        presence.dessiner()
        _grab(presence.racine, DEST_REPO / f"{PREFIX}-6-overlay-parole.png")
        _sauver(DEST_REPO / f"{PREFIX}-6-overlay-parole.png")
        presence.appliquer_etat("escalade", niveau=None, source="demo")
        presence.dessiner()
        _grab(presence.racine, DEST_REPO / f"{PREFIX}-7-overlay-escalade.png")
        _sauver(DEST_REPO / f"{PREFIX}-7-overlay-escalade.png")
    finally:
        presence.fermer()
